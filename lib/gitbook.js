...
GitBook.prototype.chapter = function(slug, cb) {
  ...
  this.chapters(function(err, chapters) {
    ...
    //if (!chapter) {
    //  if (slug == 'README') { return readChapter({ path: self._readme, title: self.title }, 'md', self._root, cb); }
    //  return cb();
    //}
    
    if (!chapter) { return cb(null, undefined); }
    
    readChapter(chapter, ext.slice(1), self._root, cb);
  });
};
...
GitBook.prototype.preface = function(cb) {
  ...
  this.chapter('README', function(err, chapter) {
    if (err) { return cb(err); }
    if (chapter) { return cb(null, chapter); }
    console.log('*** TODO: PREFACE NOT IN SUMMARY ***');
    return readChapter({ path: self._readme, title: self.title }, 'md', self._root, cb);
  });
};


function readChapter(chapter, format, root, cb) {
  var fpath = path.resolve(root, chapter.path);
