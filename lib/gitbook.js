...
GitBook.prototype.chapter = function(slug, cb) {
  ...
  this.chapters(function(err, chapters) {
    ...
    loadChapter(self._root, chapter, ext.slice(1), cb);
  });
};
...
GitBook.prototype.preface = function(cb) {
  ...
  this.chapter(base, function(err, chapter) {
    ...
    return loadChapter(self._root, { path: self._readme, title: self.title }, ext.slice(1), cb);
  });
};


function loadChapter(root, chapter, format, cb) {
