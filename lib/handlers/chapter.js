...
exports = module.exports = function(book, layout) {
  ...
  function compileChapter(page, next) {
    ...
    book.chapter(slug, function(err, chapter) {
      ...
      // FIXME: Don't merge to locals, merge to page variable
      // https://github.com/GitbookIO/gitbook/blob/master/docs/pages.md#front-matter
      merge(page.locals, chapter.front);
      ...
      // TODO: Don't use front here
      page.locals.page = {
        title: chapter.front.title || chapter.title
      };
      
      page.locals.title
        = book.title
        + (slug !== 'index' ? ': ' + (chapter.front.title || chapter.title) : '');
